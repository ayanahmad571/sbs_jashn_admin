	...
	function sqlfetchselect($columnumber,$col,$tablename,$conn){
		...
		if ($result->num_rows > 0) {
			// output data of each row
				...
				echo  '<form action="update-exec.php" method="get"">
							<tr>
							...
							<td><textarea name="4" placeholder="'.$col[4].'">'.$row[4].'</textarea><input name="?'.$col[0].'='.$row[0].'&'.$tablename.'&" type="hidden" /><input type="submit" name="submit" /></td>
							</tr>
							</form>';
			}
				...
				echo  '<tr>
							...
							<td><textarea name="5" placeholder="'.$col[5].'">'.$row[5].'</textarea></td>
							</tr>';
			}
							}
							if($columnumber == 4){
	 ...
	 $sqlcol = "SHOW COLUMNS FROM `".$tablename."`";
					...
					if ($resultcol->num_rows > 0) {
						...
						while($rowcol = $resultcol->fetch_assoc()) {
							...
							$finalcol[] = "`".$rowcol['Field']."`='".$posted[$x]."'";
							$x++;
